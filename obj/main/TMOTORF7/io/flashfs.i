...
# 46 "./src/main/io/flashfs.c" 2

static const flashPartition_t *flashPartition = 
# 47 "./src/main/io/flashfs.c" 3 4
                                               ((void *)0)
# 47 "./src/main/io/flashfs.c"
                                                   ;
static const flashGeometry_t *flashGeometry = 
# 48 "./src/main/io/flashfs.c" 3 4
                                             ((void *)0)
# 48 "./src/main/io/flashfs.c"
                                                 ;
static uint32_t flashfsSize = 0;

static __attribute__ ((section(".fastram_bss"), aligned(4))) uint8_t flashWriteBuffer[128];
# 63 "./src/main/io/flashfs.c"
static uint8_t bufferHead = 0;
static volatile uint8_t bufferTail = 0;
# 73 "./src/main/io/flashfs.c"
static volatile 
# 73 "./src/main/io/flashfs.c" 3 4
               _Bool 
# 73 "./src/main/io/flashfs.c"
                    dataWritten = 
# 73 "./src/main/io/flashfs.c" 3 4
...
static 
# 95 "./src/main/io/flashfs.c" 3 4
      _Bool 
# 95 "./src/main/io/flashfs.c"
...
void flashfsEraseCompletely(void)
{
    if (flashGeometry->sectors > 0 && flashPartitionCount() > 0) {
        ...
        const 
# 109 "./src/main/io/flashfs.c" 3 4
             _Bool 
# 109 "./src/main/io/flashfs.c"
                  doFullErase = (flashPartitionCount() == 1) && ((flashPartition->endSector + 1 - flashPartition->startSector) == flashGeometry->sectors);
        if (doFullErase) {
...
# 161 "./src/main/io/flashfs.c" 3 4
_Bool 
# 161 "./src/main/io/flashfs.c"
    flashfsIsReady(void)
{
...
# 168 "./src/main/io/flashfs.c" 3 4
_Bool 
# 168 "./src/main/io/flashfs.c"
    flashfsIsSupported(void)
{
...
void flashfsWriteCallback(uint32_t arg)
{
    ...
    dataWritten = 
# 243 "./src/main/io/flashfs.c" 3 4
                 1
# 243 "./src/main/io/flashfs.c"
                     ;
}

static uint32_t flashfsWriteBuffers(uint8_t const **buffers, uint32_t *bufferSizes, int bufferCount, 
# 246 "./src/main/io/flashfs.c" 3 4
                                                                                                    _Bool 
# 246 "./src/main/io/flashfs.c"
                                                                                                         sync)
{
    ...
    dataWritten = 
# 275 "./src/main/io/flashfs.c" 3 4
                 0
# 275 "./src/main/io/flashfs.c"
                      ;
...
# 347 "./src/main/io/flashfs.c" 3 4
_Bool 
# 347 "./src/main/io/flashfs.c"
    flashfsFlushAsync(
# 347 "./src/main/io/flashfs.c" 3 4
                      _Bool 
# 347 "./src/main/io/flashfs.c"
                           force)
{
    ...
    if (flashfsBufferIsEmpty()) {
        return 
# 354 "./src/main/io/flashfs.c" 3 4
              1
# 354 "./src/main/io/flashfs.c"
                  ;
    }
    ...
    if (!flashfsNewData()) {
        ...
        return 
# 359 "./src/main/io/flashfs.c" 3 4
              0
# 359 "./src/main/io/flashfs.c"
                   ;
    }
# 376 "./src/main/io/flashfs.c"
    bufCount = flashfsGetDirtyDataBuffers(buffers, bufferSizes);
    uint32_t bufferedBytes = bufferSizes[0] + bufferSizes[1];

    if (bufCount && (force || (bufferedBytes >= 64))) {
        flashfsWriteBuffers(buffers, bufferSizes, bufCount, 
# 380 "./src/main/io/flashfs.c" 3 4
                                                           0
# 380 "./src/main/io/flashfs.c"
                                                                );
    }
...
void flashfsFlushSync(void)
{
    ...
    if (bufCount) {
        flashfsWriteBuffers(buffers, bufferSizes, bufCount, 
# 404 "./src/main/io/flashfs.c" 3 4
                                                           1
# 404 "./src/main/io/flashfs.c"
                                                               );
    }
...
void flashfsWriteByte(uint8_t byte)
{
    ...
    if (flashfsTransmitBufferUsed() >= 64) {
        flashfsFlushAsync(
# 433 "./src/main/io/flashfs.c" 3 4
                         0
# 433 "./src/main/io/flashfs.c"
                              );
    }
}
...
void flashfsWrite(const uint8_t *data, unsigned int len, 
# 443 "./src/main/io/flashfs.c" 3 4
                                                        _Bool 
# 443 "./src/main/io/flashfs.c"
                                                             sync)
{
    uint8_t const * buffers[2];
    uint32_t bufferSizes[2];
...
# 505 "./src/main/io/flashfs.c"
    ...
    _Static_assert((FREE_BLOCK_SIZE >= 2048), "FREE_BLOCK_SIZE_too_small");

    static __attribute__ ((section(".fastram_data"), aligned(4))) union {
        uint8_t bytes[FREE_BLOCK_TEST_SIZE_BYTES];
        uint32_t ints[FREE_BLOCK_TEST_SIZE_INTS];
    } testBuffer;

    int left = 0;
    int right = flashfsSize / FREE_BLOCK_SIZE;
    int mid;
    int result = right;
    int i;
...
# 528 "./src/main/io/flashfs.c"
    ...
    while (left < right) {
        ...
        blockErased = 
# 539 "./src/main/io/flashfs.c" 3 4
                     1
# 539 "./src/main/io/flashfs.c"
                         ;
        for (i = 0; i < FREE_BLOCK_TEST_SIZE_INTS; i++) {
            if (testBuffer.ints[i] != 0xFFFFFFFF) {
                blockErased = 
# 542 "./src/main/io/flashfs.c" 3 4
                             0
# 542 "./src/main/io/flashfs.c"
                                  ;
                break;
            }
        }
...
# 565 "./src/main/io/flashfs.c" 3 4
_Bool 
# 565 "./src/main/io/flashfs.c"
    flashfsIsEOF(void)
{
...
void flashfsInit(void)
{
    ...
    flashfsSize = (flashPartition->endSector + 1 - flashPartition->startSector) * flashGeometry->sectorSize;
