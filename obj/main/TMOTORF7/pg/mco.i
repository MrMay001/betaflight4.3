...
# 32 "./src/main/pg/mco.c" 2

mcoConfig_t mcoConfig_SystemArray[2]; mcoConfig_t mcoConfig_CopyArray[2]; uint32_t mcoConfig_fnv_hash; extern const pgRegistry_t mcoConfig_Registry; const pgRegistry_t mcoConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 541 | (0 << 12), .length = 2, .size = (sizeof(mcoConfig_t) * 2) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&mcoConfig_SystemArray, .fnv_hash = &mcoConfig_fnv_hash, .copy = (uint8_t*)&mcoConfig_CopyArray, .ptr = 0, .reset = {.ptr = 0}, };
