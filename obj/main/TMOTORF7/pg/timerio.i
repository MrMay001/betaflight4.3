...
# 28 "./src/main/pg/timerio.c" 2




extern void pgResetFn_timerIOConfig(timerIOConfig_t *); timerIOConfig_t timerIOConfig_SystemArray[21]; timerIOConfig_t timerIOConfig_CopyArray[21]; uint32_t timerIOConfig_fnv_hash; extern const pgRegistry_t timerIOConfig_Registry; const pgRegistry_t timerIOConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 534 | (0 << 12), .length = 21, .size = (sizeof(timerIOConfig_t) * 21) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&timerIOConfig_SystemArray, .fnv_hash = &timerIOConfig_fnv_hash, .copy = (uint8_t*)&timerIOConfig_CopyArray, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_timerIOConfig}, };

void pgResetFn_timerIOConfig(timerIOConfig_t *config)
{

    unsigned configIndex = 0;
    for (unsigned timerIndex = 0; timerIndex < 9; timerIndex++) {
        const timerHardware_t *configuredTimer = &timerHardware[timerIndex];
        unsigned positionIndex = 1;
        for (unsigned fullTimerIndex = 0; fullTimerIndex < 78; fullTimerIndex++) {
