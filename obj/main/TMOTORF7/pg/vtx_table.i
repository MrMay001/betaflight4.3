...
extern void pgResetFn_vtxTableConfig(vtxTableConfig_t *); vtxTableConfig_t vtxTableConfig_System; vtxTableConfig_t vtxTableConfig_Copy; uint32_t vtxTableConfig_fnv_hash; extern const pgRegistry_t vtxTableConfig_Registry; const pgRegistry_t vtxTableConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 546 | (0 << 12), .length = 1, .size = sizeof(vtxTableConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&vtxTableConfig_System, .fnv_hash = &vtxTableConfig_fnv_hash, .copy = (uint8_t*)&vtxTableConfig_Copy, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_vtxTableConfig }, };
