# 1 "./src/main/common/sdft.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./src/main/common/sdft.c"
# 21 "./src/main/common/sdft.c"
...
# 28 "./src/main/common/sdft.c" 2



static __attribute__ ((section(".fastram_bss"), aligned(4))) float rPowerN;
static __attribute__ ((section(".fastram_bss"), aligned(4))) 
# 32 "./src/main/common/sdft.c" 3 4
                          _Bool 
# 32 "./src/main/common/sdft.c"
                                    isInitialized;
static __attribute__ ((section(".fastram_bss"), aligned(4))) complex_t twiddle[(72 / 2)];
...
{
    if (!isInitialized) {
        rPowerN = powf(0.9999f, 72);
        const float c = 2.0f * 3.14159265358979323846f / (float)72;
        float phi = 0.0f;
        for (int i = 0; i < (72 / 2); i++) {
            phi = c * i;
            twiddle[i] = 0.9999f * (cos_approx(phi) + 
# 46 "./src/main/common/sdft.c" 3
                                                    1.0fi 
# 46 "./src/main/common/sdft.c"
                                                               * sin_approx(phi));
        }
        isInitialized = 
# 48 "./src/main/common/sdft.c" 3 4
                       1
# 48 "./src/main/common/sdft.c"
                           ;
    }
    ...
    sdft->startBin = constrain(startBin - 1, 0, (72 / 2) - 1);
    sdft->endBin = constrain(endBin + 1, sdft->startBin, (72 / 2) - 1);

    sdft->numBatches = __extension__ ({ __typeof__ (numBatches) _a = (numBatches); __typeof__ (1) _b = (1); _a > _b ? _a : _b; });
    sdft->batchSize = (sdft->endBin - sdft->startBin) / sdft->numBatches + 1;
...
__attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void sdftPush(sdft_t *sdft, const float sample)
{
    const float delta = sample - rPowerN * sdft->samples[sdft->idx];

    sdft->samples[sdft->idx] = sample;
    sdft->idx = (sdft->idx + 1) % 72;
...
__attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void sdftPushBatch(sdft_t* sdft, const float sample, const int batchIdx)
{
...
__attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void sdftMagSq(const sdft_t *sdft, float *output)
{
    float re;
    float im;
...
__attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void sdftMagnitude(const sdft_t *sdft, float *output)
{
...
__attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void sdftWinSq(const sdft_t *sdft, float *output)
{
    complex_t val;
    float re;
    float im;
...
__attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void sdftWindow(const sdft_t *sdft, float *output)
{
...
static __attribute__((section(".tcm_code"))) __attribute__((optimize("-O2"))) void applySqrt(const sdft_t *sdft, float *data)
{
    for (int i = sdft->startBin; i <= sdft->endBin; i++) {
        data[i] = sqrtf(data[i]);
    }
