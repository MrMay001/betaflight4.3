...
# 34 "./src/main/telemetry/srxl.c" 2
...
# 50 "./src/main/telemetry/srxl.c" 2
...
# 53 "./src/main/telemetry/srxl.c" 2
...
# 59 "./src/main/telemetry/srxl.c" 2
...
void srxl2Bind(void);
# 62 "./src/main/telemetry/srxl.c" 2
...
# 66 "./src/main/telemetry/srxl.c" 2
...
# 68 "./src/main/telemetry/srxl.c" 2
...
# 70 "./src/main/telemetry/srxl.c" 2
# 84 "./src/main/telemetry/srxl.c"
static 
# 84 "./src/main/telemetry/srxl.c" 3 4
      _Bool 
# 84 "./src/main/telemetry/srxl.c"
           srxlTelemetryEnabled;
static 
# 85 "./src/main/telemetry/srxl.c" 3 4
      _Bool 
# 85 "./src/main/telemetry/srxl.c"
           srxl2 = 
# 85 "./src/main/telemetry/srxl.c" 3 4
                   0
# 85 "./src/main/telemetry/srxl.c"
...
# 142 "./src/main/telemetry/srxl.c"

# 142 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 142 "./src/main/telemetry/srxl.c"
    srxlFrameQos(sbuf_t *dst, timeUs_t currentTimeUs)
{
    ...
    return 
# 152 "./src/main/telemetry/srxl.c" 3 4
          1
# 152 "./src/main/telemetry/srxl.c"
              ;
}
# 181 "./src/main/telemetry/srxl.c"
uint16_t getMotorAveragePeriod(void)
{


    uint32_t rpm = 0;
    uint16_t period_us = 0xffff;


    escSensorData_t *escData = getEscSensorData(255);
    if (escData != 
# 190 "./src/main/telemetry/srxl.c" 3 4
                  ((void *)0)
# 190 "./src/main/telemetry/srxl.c"
                      ) {
        rpm = escData->rpm;
    }
    ...
    if (useDshotTelemetry) {
        ...
        if (motors > 0) {
            ...
            rpm = 100.0f / (motorConfig()->motorPoleCount / 2.0f) * rpm;
            rpm /= motors;
        }
    }


    if (rpm > 999 && rpm < 60000000) {
        period_us = 60000000 / rpm;
    } else {
        period_us = 60000000 / 999;
    }
...
# 221 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 221 "./src/main/telemetry/srxl.c"
    srxlFrameRpm(sbuf_t *dst, timeUs_t currentTimeUs)
{
    int16_t coreTemp = 0x7fff;

    coreTemp = getCoreTemperatureCelsius();
    coreTemp = coreTemp * 9 / 5 + 32;
    ...
    sbufWriteU8(dst, 0x00);
    sbufWriteU16BigEndian(dst, getMotorAveragePeriod());
    if (telemetryConfig()->report_cell_voltage) {
        sbufWriteU16BigEndian(dst, getBatteryAverageCellVoltage());
    } else {
        sbufWriteU16BigEndian(dst, getBatteryVoltage());
    }
    sbufWriteU16BigEndian(dst, coreTemp);
    sbufFill(dst, 0xff, 8);


    return 
# 243 "./src/main/telemetry/srxl.c" 3 4
          1
# 243 "./src/main/telemetry/srxl.c"
              ;
}
...
# 297 "./src/main/telemetry/srxl.c"

# 297 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 297 "./src/main/telemetry/srxl.c"
    srxlFrameGpsLoc(sbuf_t *dst, timeUs_t currentTimeUs)
{
    ((void)(currentTimeUs));
    ...
    if (!featureIsEnabled(FEATURE_GPS) || !(stateFlags & (GPS_FIX)) || gpsSol.numSat < 6) {
        return 
# 306 "./src/main/telemetry/srxl.c" 3 4
              0
# 306 "./src/main/telemetry/srxl.c"
                   ;
    }
    ...
    altitudeLo = __extension__ ({ __typeof__ (gpsSol.llh.altCm) _x = (gpsSol.llh.altCm); _x > 0 ? _x : -_x; }) / 10;
    ...
    gpsFlags = 0x10 | 0x08 | 0x20;
    gpsFlags |= (gpsSol.llh.lat > 0) ? 0x01 : 0;
    gpsFlags |= (gpsSol.llh.lon > 0) ? 0x02 : 0;
    gpsFlags |= (gpsSol.llh.altCm < 0) ? 0x80 : 0;
    gpsFlags |= (gpsSol.llh.lon / 10000000L > 99) ? 0x04 : 0;


    sbufWriteU8(dst, 0x16);
    sbufWriteU8(dst, 0x00);
    ...
    return 
# 346 "./src/main/telemetry/srxl.c" 3 4
          1
# 346 "./src/main/telemetry/srxl.c"
              ;
}
# 365 "./src/main/telemetry/srxl.c"

# 365 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 365 "./src/main/telemetry/srxl.c"
    srxlFrameGpsStat(sbuf_t *dst, timeUs_t currentTimeUs)
{
    ((void)(currentTimeUs));
    uint32_t timeBcd;
...
# 371 "./src/main/telemetry/srxl.c" 3 4
   _Bool 
# 371 "./src/main/telemetry/srxl.c"
        timeProvided = 
# 371 "./src/main/telemetry/srxl.c" 3 4
                       0
# 371 "./src/main/telemetry/srxl.c"
    ...
    if (!featureIsEnabled(FEATURE_GPS) || !(stateFlags & (GPS_FIX)) || gpsSol.numSat < 6) {
        return 
# 374 "./src/main/telemetry/srxl.c" 3 4
              0
# 374 "./src/main/telemetry/srxl.c"
                   ;
    }
    ...
    if (rtcHasTime()) {
        ...
        timeProvided = 
# 397 "./src/main/telemetry/srxl.c" 3 4
                      1
# 397 "./src/main/telemetry/srxl.c"
                          ;
    }
    ...
    return 
# 411 "./src/main/telemetry/srxl.c" 3 4
          1
# 411 "./src/main/telemetry/srxl.c"
              ;
}
# 438 "./src/main/telemetry/srxl.c"

# 438 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 438 "./src/main/telemetry/srxl.c"
    srxlFrameFlightPackCurrent(sbuf_t *dst, timeUs_t currentTimeUs)
{
    ...
    if ( amps != sentAmps ||
        ...
        return 
# 466 "./src/main/telemetry/srxl.c" 3 4
              1
# 466 "./src/main/telemetry/srxl.c"
                  ;
    }
    return 
# 468 "./src/main/telemetry/srxl.c" 3 4
          0
# 468 "./src/main/telemetry/srxl.c"
               ;
}
# 493 "./src/main/telemetry/srxl.c"
static char srxlTextBuff[(9)][(13)];
static 
# 494 "./src/main/telemetry/srxl.c" 3 4
      _Bool 
# 494 "./src/main/telemetry/srxl.c"
           lineSent[(9)];
...
int spektrumTmTextGenPutChar(uint8_t col, uint8_t row, char c)
{
    if (row < 9 && col < 12) {
        ...
        if (srxlTextBuff[row][col] != c) {
          ...
          lineSent[row] = 
# 505 "./src/main/telemetry/srxl.c" 3 4
                         0
# 505 "./src/main/telemetry/srxl.c"
                              ;
        }
    }
    return 0;
...
# 512 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 512 "./src/main/telemetry/srxl.c"
    srxlFrameText(sbuf_t *dst, timeUs_t currentTimeUs)
{
    ...
    lineSent[lineNo] = 
# 530 "./src/main/telemetry/srxl.c" 3 4
                      1
# 530 "./src/main/telemetry/srxl.c"
                          ;
    lineNo = (lineNo + 1) % (9);
    ...
    return 
# 536 "./src/main/telemetry/srxl.c" 3 4
          1
# 536 "./src/main/telemetry/srxl.c"
              ;
}
...
static void collectVtxTmData(spektrumVtx_t * vtx)
{
    ...
    unsigned vtxStatus;
    if (vtxDevice == 
# 551 "./src/main/telemetry/srxl.c" 3 4
                    ((void *)0) 
# 551 "./src/main/telemetry/srxl.c"
...
# 574 "./src/main/telemetry/srxl.c" 3 4
                                         ((void *)0)
# 574 "./src/main/telemetry/srxl.c"
                                             ;
        ...
        if (powerIndexTable != 
# 602 "./src/main/telemetry/srxl.c" 3 4
                              ((void *)0)
# 602 "./src/main/telemetry/srxl.c"
                                  ) {
            for (int i = 0; i < 8; i++)
...
# 648 "./src/main/telemetry/srxl.c"
static 
# 648 "./src/main/telemetry/srxl.c" 3 4
      _Bool 
# 648 "./src/main/telemetry/srxl.c"
           srxlFrameVTX(sbuf_t *dst, timeUs_t currentTimeUs)
{
    ...
    if ((vtxDeviceType != VTXDEV_UNKNOWN) && vtxDeviceType != VTXDEV_UNSUPPORTED) {
        ...
        if ((memcmp(&vtxSent, &vtx, sizeof(spektrumVtx_t)) != 0) ||
            ...
            return 
# 675 "./src/main/telemetry/srxl.c" 3 4
                  1
# 675 "./src/main/telemetry/srxl.c"
                      ;
        }
    }
    return 
# 678 "./src/main/telemetry/srxl.c" 3 4
          0
# 678 "./src/main/telemetry/srxl.c"
               ;
}
# 716 "./src/main/telemetry/srxl.c"
typedef 
# 716 "./src/main/telemetry/srxl.c" 3 4
       _Bool 
# 716 "./src/main/telemetry/srxl.c"
            (*srxlScheduleFnPtr)(sbuf_t *dst, timeUs_t currentTimeUs);

const srxlScheduleFnPtr srxlScheduleFuncs[(2 + (1 + 1 + 1 + 1 + 1))] = {
...
static void processSrxl(timeUs_t currentTimeUs)
{
    ...
    if (srxlScheduleIndex < 2) {
        ...
        srxlScheduleUserIndex = (srxlScheduleUserIndex + 1) % (1 + 1 + 1 + 1 + 1);
...
void initSrxlTelemetry(void)
{
    ...
    if (srxlRxIsActive()) {
        srxlTelemetryEnabled = 
# 776 "./src/main/telemetry/srxl.c" 3 4
                              1
# 776 "./src/main/telemetry/srxl.c"
                                  ;
        srxl2 = 
# 777 "./src/main/telemetry/srxl.c" 3 4
               0
# 777 "./src/main/telemetry/srxl.c"
                    ;

    } else if (srxl2RxIsActive()) {
        srxlTelemetryEnabled = 
# 780 "./src/main/telemetry/srxl.c" 3 4
                              1
# 780 "./src/main/telemetry/srxl.c"
                                  ;
        srxl2 = 
# 781 "./src/main/telemetry/srxl.c" 3 4
               1
# 781 "./src/main/telemetry/srxl.c"
                   ;

    } else {
        srxlTelemetryEnabled = 
# 784 "./src/main/telemetry/srxl.c" 3 4
                              0
# 784 "./src/main/telemetry/srxl.c"
                                   ;
        srxl2 = 
# 785 "./src/main/telemetry/srxl.c" 3 4
               0
# 785 "./src/main/telemetry/srxl.c"
                    ;
    }
...
# 795 "./src/main/telemetry/srxl.c" 3 4
_Bool 
# 795 "./src/main/telemetry/srxl.c"
    checkSrxlTelemetryState(void)
{
