...
# 23 "./src/main/telemetry/ghst.c" 2
...
# 24 "./src/main/telemetry/ghst.c" 2
...
static 
# 74 "./src/main/telemetry/ghst.c" 3 4
      _Bool 
# 74 "./src/main/telemetry/ghst.c"
...
{
    crc8_sbuf_append(dst, &ghstFrame[2], 0xD5);
...
void ghstFrameGpsSecondaryTelemetry(sbuf_t *dst)
{
    ...
    sbufWriteU16(dst, (uint16_t) (GPS_distanceToHome / 10));
...
void ghstFrameMagBaro(sbuf_t *dst)
{
    ...
    sbufWriteU16(dst, yaw);
    sbufWriteU16(dst, altitude);
    sbufWriteU8(dst, vario);
...
static void processGhst(void)
{
    ...
    if (currentSchedule & (1 << (GHST_FRAME_GPS_PRIMARY_INDEX))) {
        ghstInitializeFrame(dst);
        ghstFrameGpsPrimaryTelemetry(dst);
        ghstFinalize(dst);
    }

    if (currentSchedule & (1 << (GHST_FRAME_GPS_SECONDARY_INDEX))) {
        ghstInitializeFrame(dst);
    ...
    if (currentSchedule & (1 << (GHST_FRAME_MAGBARO_INDEX))) {
        ghstInitializeFrame(dst);
...
void initGhstTelemetry(void)
{
    ...
    if (featureIsEnabled(FEATURE_GPS)
       && telemetryIsSensorEnabled(SENSOR_ALTITUDE | SENSOR_LAT_LONG)) {
        ghstSchedule[index++] = (1 << (GHST_FRAME_GPS_PRIMARY_INDEX));
    }

    if (featureIsEnabled(FEATURE_GPS)
       && telemetryIsSensorEnabled(SENSOR_GROUND_SPEED | SENSOR_HEADING)) {
        ghstSchedule[index++] = (1 << (GHST_FRAME_GPS_SECONDARY_INDEX));
     }
    ...
    if ((sensors(SENSOR_BARO) && telemetryIsSensorEnabled(SENSOR_ALTITUDE))
        ...
        ghstSchedule[index++] = (1 << (GHST_FRAME_MAGBARO_INDEX));
    }
...
# 291 "./src/main/telemetry/ghst.c" 3 4
_Bool 
# 291 "./src/main/telemetry/ghst.c"
    checkGhstTelemetryState(void)
{
