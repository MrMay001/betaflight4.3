...
# 53 "./src/main/rx/crsf.c" 2
# 66 "./src/main/rx/crsf.c"
static 
# 66 "./src/main/rx/crsf.c" 3 4
                  _Bool 
# 66 "./src/main/rx/crsf.c"
                       crsfFrameDone = 
# 66 "./src/main/rx/crsf.c" 3 4
...
static float channelScale = 0.62477120195241f;




const uint16_t uplinkTXPowerStatesMw[9] = {0, 10, 25, 100, 500, 1000, 2000, 250, 50};
# 110 "./src/main/rx/crsf.c"
...
static void handleCrsfLinkStatisticsFrame(const crsfLinkStatistics_t* statsPtr, timeUs_t currentTimeUs)
{
    ...
    if (rssiSource == RSSI_SOURCE_RX_PROTOCOL_CRSF) {
        if (rxConfig()->crsf_use_rx_snr) {
            ...
            const uint16_t rsnrPercentScaled = constrain((stats.uplink_SNR + 10) * 20, 0, 1023);
            setRssi(rsnrPercentScaled, RSSI_SOURCE_RX_PROTOCOL_CRSF);

            rssiDbm = stats.uplink_SNR;

        } else {
            const uint16_t rssiPercentScaled = scaleRange(rssiDbm, (-130), 0, 0, 1023);
    ...
    const uint8_t crsfUplinkPowerStatesItemIndex = (stats.uplink_TX_Power < 9) ? stats.uplink_TX_Power : 0;
    rxSetUplinkTxPwrMw(uplinkTXPowerStatesMw[crsfUplinkPowerStatesItemIndex]);


    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(0)] = (stats.uplink_RSSI_1); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(1)] = (stats.uplink_RSSI_2); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(2)] = (stats.uplink_Link_quality); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(3)] = (stats.rf_Mode); } } while (0);

    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_PWR)) { debug[(0)] = (stats.active_antenna); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_PWR)) { debug[(1)] = (stats.uplink_SNR); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_PWR)) { debug[(2)] = (stats.uplink_TX_Power); } } while (0);

    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_DOWN)) { debug[(0)] = (stats.downlink_RSSI); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_DOWN)) { debug[(1)] = (stats.downlink_Link_quality); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_DOWN)) { debug[(2)] = (stats.downlink_SNR); } } while (0);
}
...
static void handleCrsfLinkStatisticsTxFrame(const crsfLinkStatisticsTx_t* statsPtr, timeUs_t currentTimeUs)
{
    ...
    lastLinkStatisticsFrameUs = currentTimeUs;
    if (rssiSource == RSSI_SOURCE_RX_PROTOCOL_CRSF) {
        const uint16_t rssiPercentScaled = scaleRange(stats.uplink_RSSI_percentage, 0, 100, 0, 1023);
        setRssi(rssiPercentScaled, RSSI_SOURCE_RX_PROTOCOL_CRSF);
    }
    ...
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(0)] = (stats.uplink_RSSI); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(1)] = (stats.uplink_SNR); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(2)] = (stats.uplink_Link_quality); } } while (0);
    do { if (debugMode == (DEBUG_CRSF_LINK_STATISTICS_UPLINK)) { debug[(3)] = (stats.uplink_RSSI_percentage); } } while (0);
}
...
static void crsfCheckRssi(uint32_t currentTimeUs) {
    ...
    if (cmpTimeUs(currentTimeUs, lastLinkStatisticsFrameUs) > 250000) {
        if (rssiSource == RSSI_SOURCE_RX_PROTOCOL_CRSF) {
            setRssiDirect(0, RSSI_SOURCE_RX_PROTOCOL_CRSF);

            if (rxConfig()->crsf_use_rx_snr) {
                setRssiDbmDirect((-30), RSSI_SOURCE_RX_PROTOCOL_CRSF);
            } else {
...
static uint8_t crsfFrameCRC(void)
{
    ...
    uint8_t crc = crc8_calc(0, crsfFrame.frame.type, 0xD5);
    for (int ii = 0; ii < crsfFrame.frame.frameLength - CRSF_FRAME_LENGTH_TYPE_CRC; ++ii) {
        crc = crc8_calc(crc, crsfFrame.frame.payload[ii], 0xD5);
    }
    return crc;
}
...
static uint8_t crsfFrameCmdCRC(void)
{
    ...
    uint8_t crc = crc8_calc(0, crsfFrame.frame.type, 0xBA);
    for (int ii = 0; ii < crsfFrame.frame.frameLength - CRSF_FRAME_LENGTH_TYPE_CRC - 1; ++ii) {
        crc = crc8_calc(crc, crsfFrame.frame.payload[ii], 0xBA);
    }
    return crc;
}
...
static void crsfDataReceive(uint16_t c, void *data)
{
    ...
    if (cmpTimeUs(currentTimeUs, crsfFrameStartAtUs) > 1750) {
    ...
    const int fullFrameLength = crsfFramePosition < 3 ? 5 : __extension__ ({ __typeof__ (crsfFrame.frame.frameLength + CRSF_FRAME_LENGTH_ADDRESS + CRSF_FRAME_LENGTH_FRAMELENGTH) _a = (crsfFrame.frame.frameLength + CRSF_FRAME_LENGTH_ADDRESS + CRSF_FRAME_LENGTH_FRAMELENGTH); __typeof__ (CRSF_FRAME_SIZE_MAX) _b = (CRSF_FRAME_SIZE_MAX); _a < _b ? _a : _b; });
            ...
            if (crc == crsfFrame.bytes[fullFrameLength - 1]) {
                ...
                case CRSF_FRAMETYPE_SUBSET_RC_CHANNELS_PACKED:
                    if (crsfFrame.frame.deviceAddress == CRSF_ADDRESS_FLIGHT_CONTROLLER) {
                        ...
                        crsfFrameDone = 
# 399 "./src/main/rx/crsf.c" 3 4
                                       1
# 399 "./src/main/rx/crsf.c"
                                           ;
...
static uint8_t crsfFrameStatus(rxRuntimeState_t *rxRuntimeState)
{
    ...
    if (crsfFrameDone) {
        crsfFrameDone = 
# 490 "./src/main/rx/crsf.c" 3 4
                       0
# 490 "./src/main/rx/crsf.c"
                            ;
        ...
        if (crsfChannelDataFrame.frame.type == CRSF_FRAMETYPE_RC_CHANNELS_PACKED) {
            ...
            channelScale = 0.62477120195241f;
            ...
            uint8_t startChannel = configByte & 0x1F;
            configByte >>= 5;


            uint8_t channelBits;
            uint16_t channelMask;
            uint8_t channelRes = configByte & 0x03;
            configByte >>= 2;
            switch (channelRes) {
            case 0:
                channelBits = 10;
...
{
    ((void)(rxRuntimeState));
    if (channelScale == 0.62477120195241f) {
# 592 "./src/main/rx/crsf.c"
...
# 617 "./src/main/rx/crsf.c" 3 4
_Bool 
# 617 "./src/main/rx/crsf.c"
    crsfRxIsTelemetryBufEmpty(void)
{
...
# 622 "./src/main/rx/crsf.c" 3 4
_Bool 
# 622 "./src/main/rx/crsf.c"
    crsfRxInit(const rxConfig_t *rxConfig, rxRuntimeState_t *rxRuntimeState)
{
    for (int ii = 0; ii < 16; ++ii) {
        crsfChannelData[ii] = (16 * rxConfig->midrc) / 10 - 1408;
    }

    rxRuntimeState->channelCount = 16;
    rxRuntimeState->rxRefreshRate = 6667;
    ...
    if (!portConfig) {
        return 
# 637 "./src/main/rx/crsf.c" 3 4
              0
# 637 "./src/main/rx/crsf.c"
                   ;
    }
    ...
    crsfBaudrate = rxConfig->crsf_use_negotiated_baud ? getCrsfCachedBaudrate() : 420000;
    ...
    return serialPort != 
# 664 "./src/main/rx/crsf.c" 3 4
                        ((void *)0)
# 664 "./src/main/rx/crsf.c"
                            ;
}
...
# 674 "./src/main/rx/crsf.c" 3 4
_Bool 
# 674 "./src/main/rx/crsf.c"
    crsfRxUseNegotiatedBaud(void)
{
...
# 680 "./src/main/rx/crsf.c" 3 4
_Bool 
# 680 "./src/main/rx/crsf.c"
    crsfRxIsActive(void)
{
    return serialPort != 
# 682 "./src/main/rx/crsf.c" 3 4
                        ((void *)0)
# 682 "./src/main/rx/crsf.c"
                            ;
}
