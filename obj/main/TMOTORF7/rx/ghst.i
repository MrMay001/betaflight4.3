...
# 50 "./src/main/rx/ghst.c" 2
# 68 "./src/main/rx/ghst.c"
static volatile 
# 68 "./src/main/rx/ghst.c" 3 4
                           _Bool 
# 68 "./src/main/rx/ghst.c"
                                ghstFrameAvailable = 
# 68 "./src/main/rx/ghst.c" 3 4
...
# 68 "./src/main/rx/ghst.c"
                                                          ;
static volatile 
# 69 "./src/main/rx/ghst.c" 3 4
                           _Bool 
# 69 "./src/main/rx/ghst.c"
                                ghstValidatedFrameAvailable = 
# 69 "./src/main/rx/ghst.c" 3 4
                                                              0
# 69 "./src/main/rx/ghst.c"
                                                                   ;
static volatile 
# 70 "./src/main/rx/ghst.c" 3 4
                           _Bool 
# 70 "./src/main/rx/ghst.c"
                                ghstTransmittingTelemetry = 
# 70 "./src/main/rx/ghst.c" 3 4
                                                            0
# 70 "./src/main/rx/ghst.c"
                                                                 ;

static ghstFrame_t ghstIncomingFrame;
static ghstFrame_t ghstValidatedFrame;

static uint32_t ghstChannelData[16];
...
static uint8_t telemetryBuf[24];
static uint8_t telemetryBufLen = 0;
# 114 "./src/main/rx/ghst.c"
void ghstRxWriteTelemetryData(const void *data, int len)
{
...
static uint8_t ghstFrameCRC(ghstFrame_t *pGhstFrame)
{

    uint8_t crc = crc8_calc(0, pGhstFrame->frame.type, 0xD5);
    for (int i = 0; i < pGhstFrame->frame.len - 1 - 1; ++i) {
        crc = crc8_calc(crc, pGhstFrame->frame.payload[i], 0xD5);
    }
    return crc;
}


static void ghstDataReceive(uint16_t c, void *data)
{
    ((void)(data));
    ...
    if (cmpTimeUs(currentTimeUs, ghstRxFrameStartAtUs) > 500) {
    ...
    const int fullFrameLength = ghstFrameIdx < 3 ? 5 : ghstIncomingFrame.frame.len + 1 + 1;
    ...
    if (ghstFrameIdx < fullFrameLength) {
        ...
        if (ghstFrameIdx >= fullFrameLength) {
            ...
            ghstFrameAvailable = 
# 170 "./src/main/rx/ghst.c" 3 4
                                1
# 170 "./src/main/rx/ghst.c"
                                    ;
...
static 
# 178 "./src/main/rx/ghst.c" 3 4
      _Bool 
# 178 "./src/main/rx/ghst.c"
           shouldSendTelemetryFrame(void)
{
    const timeUs_t now = micros();
    const timeUs_t timeSinceRxFrameEndUs = cmpTimeUs(now, ghstRxFrameEndAtUs);
    return telemetryBufLen > 0 && timeSinceRxFrameEndUs > 1000 && timeSinceRxFrameEndUs < 2000;
}

static uint8_t ghstFrameStatus(rxRuntimeState_t *rxRuntimeState)
{
    ((void)(rxRuntimeState));
    ...
    if (ghstFrameAvailable) {
        ghstFrameAvailable = 
# 191 "./src/main/rx/ghst.c" 3 4
                            0
# 191 "./src/main/rx/ghst.c"
                                 ;

        const uint8_t crc = ghstFrameCRC(&ghstValidatedFrame);
        const int fullFrameLength = ghstValidatedFrame.frame.len + 1 + 1;
        if (crc == ghstValidatedFrame.bytes[fullFrameLength - 1] && ghstValidatedFrame.frame.addr == GHST_ADDR_FC) {
            ghstValidatedFrameAvailable = 
# 196 "./src/main/rx/ghst.c" 3 4
                                         1
# 196 "./src/main/rx/ghst.c"
                                             ;
            rxRuntimeState->lastRcFrameTimeUs = ghstRxFrameEndAtUs;
            return RX_FRAME_COMPLETE | RX_FRAME_PROCESSING_REQUIRED;
        }

        if (crc != ghstValidatedFrame.bytes[fullFrameLength - 1]) {
            do { if (debugMode == (DEBUG_GHST)) { debug[(DEBUG_GHST_CRC_ERRORS)] = (++crcErrorCount); } } while (0);
        }
...
static 
# 215 "./src/main/rx/ghst.c" 3 4
      _Bool 
# 215 "./src/main/rx/ghst.c"
           ghstProcessFrame(const rxRuntimeState_t *rxRuntimeState)
{
    ...
    if (shouldSendTelemetryFrame()) {
        ghstTransmittingTelemetry = 
# 226 "./src/main/rx/ghst.c" 3 4
                                   1
# 226 "./src/main/rx/ghst.c"
                                       ;
        ...
        if (ghstValidatedFrame.frame.type >= GHST_UL_RC_CHANS_HS4_FIRST &&
            ...
            switch(ghstValidatedFrame.frame.type) {
                case GHST_UL_RC_CHANS_HS4_RSSI: {
                    ...
                    do { if (debugMode == (DEBUG_GHST)) { debug[(DEBUG_GHST_RX_RSSI)] = (-rssiFrame->rssi); } } while (0);
                    do { if (debugMode == (DEBUG_GHST)) { debug[(DEBUG_GHST_RX_LQ)] = (rssiFrame->lq); } } while (0);

                    if (rssiSource == RSSI_SOURCE_RX_PROTOCOL) {

                        uint16_t rssiPercentScaled = scaleRange(-rssiFrame->rssi, (-117), (-60), 0, 1023);
                        rssiPercentScaled = __extension__ ({ __typeof__ (rssiPercentScaled) _a = (rssiPercentScaled); __typeof__ (1023) _b = (1023); _a < _b ? _a : _b; });
                ...
                default:
                    do { if (debugMode == (DEBUG_GHST)) { debug[(DEBUG_GHST_UNKNOWN_FRAMES)] = (++unknownFrameCount); } } while (0);
                    break;
            }
    ...
    return 
# 289 "./src/main/rx/ghst.c" 3 4
          1
# 289 "./src/main/rx/ghst.c"
              ;
}

static float ghstReadRawRC(const rxRuntimeState_t *rxRuntimeState, uint8_t chan)
{
    ((void)(rxRuntimeState));
# 305 "./src/main/rx/ghst.c"
...
static void ghstIdle()
{
    if (ghstTransmittingTelemetry) {
        ghstTransmittingTelemetry = 
# 312 "./src/main/rx/ghst.c" 3 4
                                   0
# 312 "./src/main/rx/ghst.c"
                                        ;
    }
}


# 316 "./src/main/rx/ghst.c" 3 4
_Bool 
# 316 "./src/main/rx/ghst.c"
    ghstRxInit(const rxConfig_t *rxConfig, rxRuntimeState_t *rxRuntimeState)
{
    ...
    rxRuntimeState->rxRefreshRate = 4500;
    ...
    if (!portConfig) {
        return 
# 332 "./src/main/rx/ghst.c" 3 4
              0
# 332 "./src/main/rx/ghst.c"
                   ;
    }
...
# 338 "./src/main/rx/ghst.c" 3 4
       ((void *)0)
# 338 "./src/main/rx/ghst.c"
           ,
        420000,
        MODE_RXTX,
        (SERIAL_STOPBITS_1 | SERIAL_PARITY_NO | SERIAL_BIDIR | SERIAL_BIDIR_PP) | (rxConfig->serialrx_inverted ? SERIAL_INVERTED : 0)
        );
    ...
    return serialPort != 
# 355 "./src/main/rx/ghst.c" 3 4
                        ((void *)0)
# 355 "./src/main/rx/ghst.c"
                            ;
}


# 358 "./src/main/rx/ghst.c" 3 4
_Bool 
# 358 "./src/main/rx/ghst.c"
    ghstRxIsActive(void)
{
    return serialPort != 
# 360 "./src/main/rx/ghst.c" 3 4
                        ((void *)0)
# 360 "./src/main/rx/ghst.c"
                            ;
}
