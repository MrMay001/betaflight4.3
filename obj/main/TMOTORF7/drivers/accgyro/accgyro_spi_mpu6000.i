...
# 104 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c"
void mpu6000SpiGyroInit(gyroDev_t *gyro)
{
    ...
    spiWriteReg(&gyro->dev, 0x1A, mpuGyroDLPF(gyro));
    delayMicroseconds(1);

    spiSetClkDivisor(&gyro->dev, spiCalculateDivider(20000000));
...
uint8_t mpu6000SpiDetect(const extDevice_t *dev)
{
    ...
    spiWriteReg(dev, 0x6B, 0x80);
    delay(100);

    const uint8_t whoAmI = spiReadRegMsk(dev, 0x75);
    delayMicroseconds(1);
    uint8_t detectedSensor = MPU_NONE;

    if (whoAmI == (0x68)) {
        const uint8_t productID = spiReadRegMsk(dev, 0x0C);
        ...
        spiWriteReg(dev, 0x68, 0x04 | 0x02 | 0x01);
        delay(100);
    }
...
static void mpu6000AccAndGyroInit(gyroDev_t *gyro)
{
    ...
    spiWriteReg(&gyro->dev, 0x6B, 0x03);
    delayMicroseconds(15);


    spiWriteReg(&gyro->dev, 0x6A, 0x10);
    delayMicroseconds(15);

    spiWriteReg(&gyro->dev, 0x6C, 0x00);
    delayMicroseconds(15);



    spiWriteReg(&gyro->dev, 0x19, gyro->mpuDividerDrops);
    delayMicroseconds(15);


    spiWriteReg(&gyro->dev, 0x1B, INV_FSR_2000DPS << 3);
    delayMicroseconds(15);


    spiWriteReg(&gyro->dev, 0x1C, INV_FSR_16G << 3);
    delayMicroseconds(15);

    spiWriteReg(&gyro->dev, 0x37, 0 << 7 | 0 << 6 | 0 << 5 | 1 << 4 | 0 << 3 | 0 << 2 | 0 << 1 | 0 << 0);
    delayMicroseconds(15);


    spiWriteReg(&gyro->dev, 0x38, (1 << 0));
    delayMicroseconds(15);

}


# 205 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c" 3 4
_Bool 
# 205 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c"
    mpu6000SpiAccDetect(accDev_t *acc)
{
    if (acc->mpuDetectionResult.sensor != MPU_60x0_SPI) {
        return 
# 208 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c" 3 4
              0
# 208 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c"
                   ;
    }
    ...
    return 
# 214 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c" 3 4
          1
# 214 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c"
              ;
}


# 217 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c" 3 4
_Bool 
# 217 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c"
    mpu6000SpiGyroDetect(gyroDev_t *gyro)
{
    if (gyro->mpuDetectionResult.sensor != MPU_60x0_SPI) {
        return 
# 220 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c" 3 4
              0
# 220 "./src/main/drivers/accgyro/accgyro_spi_mpu6000.c"
                   ;
    }

    gyro->initFn = mpu6000SpiGyroInit;
    gyro->readFn = mpuGyroReadSPI;
    gyro->scale = (2000.0f / (1 << 15));

    gyro->gyroShortPeriod = clockMicrosToCycles(82);
